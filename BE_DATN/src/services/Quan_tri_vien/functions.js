...
exports.uploadAvatarResize = async (file, time = null, width = 500, height = 500) => {
    let file_path = file.path;
    let path_file = getPathImage(time);
...
exports.getUrlAvatar = (time, link) => {
    ...
    return `${process.env.BUILD_MODE === 'dev' ? process.env.IMG_LINK_DEV : process.env.IMG_LINK_PRODUCTION}/Quan_tri_vien/${dateTime}/${link}`
}

//Tạo đường dẫn upload ảnh
const getPathImage = (time) => {
    const dateTime = functions.convertDate(time, true);
    const path = `${process.env.IMG_PATH}/Quan_tri_vien/${dateTime}/`; // Tạo đường dẫn đến thư mục của người dùng

// TruyenCard.tsx
import React from 'react';
import { Card, Typography, Col, Button, Tooltip, notification } from 'antd';
import { useNavigate } from 'react-router-dom';
import s from './styles.module.scss';
import { POST } from '../../api/baseAPI';
const { Text } = Typography;

const TruyenCard: any = ({ truyen, checkVip = false, setIsLoading }: { truyen: any, checkVip: any, setIsLoading: any }) => {
    const navigate = useNavigate();
    const isAuthenticated = localStorage.getItem('isAuthenticated');
    const handleDetailClick = async () => {
        if (isAuthenticated === 'false') {
            notification.error({
                message: 'Hệ thống',
                description: `Vui lòng đăng nhập để có thể xem thông tin truyện`,
                duration: 3,
            });
            return;
        }
        else {
            setIsLoading(true);
            try {
                var formData = new FormData();

                formData.append("_id", String(truyen._id));

                const response = await POST('api/homepage/quan_ly_truyen/tang_so_luot_truy_cap', formData);

                if (response?.status === 200) {
                    navigate(`/chi-tiet/${truyen._id}`);
                }
                else {
                    notification.error({
                        message: response?.data?.message,
                        description: response?.data?.content,
                        duration: 3,
                    });
                }

                setIsLoading(false);
            } catch (error: any) {
                setIsLoading(false);
                notification.error({
                    message: 'Hệ thống',
                    description: error.message,
                    duration: 3,
                });
            }
        }
    };
    return (
        <Card
            hoverable
            cover={<img alt={truyen.ten} src={truyen.trangBia || '/truyen_loading.png'} className={s.coverImage} />}
            style={{ flex: 1, display: 'flex', flexDirection: 'column' }}
            className={s.card}
        >
            <div className={s.footer}>
                <Tooltip title={truyen.ten}>
                    <Text className={s.title}>{truyen.ten}</Text>
                </Tooltip>
                <Button onClick={handleDetailClick} className={s.button}>
                    Đọc truyện
                </Button>
            </div>
            <Text className={s.subInfo}>
                {truyen.soLuotTruyCap || 0} lượt truy cập
            </Text>
        </Card>
    );
};

export default TruyenCard;